...
if __name__ == "__main__":
    ...
    plot = False
    not_found = []
    for S_h_in_nm, S_v, t_l_in_minutes, as_events in itertools.product(*[[3, 6], [1000, 2000], [5, 10], [False, True]]):
        # S_h_in_nm = 3
        S_h = 1852 * S_h_in_nm
        # S_v = 1000
        t_l = t_l_in_minutes/60.
        ...
        if timeshift is not None:
            ...
            repeats = 5
            timeshift_suffix = '-as-events-repeats-{}'.format(repeats)
            title_suffix=', as events, {} replications'.format(repeats)
            conflicts_x_and_y_lim_for_plot = 80

        replay_results_file = 'data/conflict_replay_results/{}_20180101-20180102-20180104-20180105-splits_[0-1-2-3]-S_h-{}-S_v-{}-t_l-{:.4f}{}.xlsx'.format(
            filename_prefix, S_h_in_nm, S_v, t_l, timeshift_suffix)
        try:
            replay_df_all_splits = pd.read_excel(replay_results_file)
        except FileNotFoundError:
            not_found.append(replay_results_file)
            continue
        combined_df_list = []
        splits_to_consider = list(range(4))
        ...
        ax0_twin = ax[0].twiny()
        sorting = ['converging_av', 'converging_la', 'cross', 'cross_la',  'none',  'diverging_av', 'all_av', 'clustered-unclustered', 'unclustered-unclustered', 'within-cluster']
        combined_df_all['overlap_type'] = pd.Categorical(combined_df_all['overlap_type'], sorting)
        ...
        ax0_twin.legend(lines + lines2, ['Conflicts', 'Flow pairs'], loc='upper right')
        ...
        for ext in ['png', 'eps']:
            for overleaf_project_name in ['thesis_full', 'thesis_article']:
                folder = "C:/Users/salom/Dropbox/Apps/Overleaf/{}/".format(overleaf_project_name)
                filename = 'figures/replay/{}_20180101-20180102-20180104-20180105-splits_[0-1-2-3]-S_h-{}-S_v-{}-t_l-{:.4f}{}.{}'.format(filename_prefix, S_h_in_nm, S_v, t_l, timeshift_suffix, ext)
        ...
        df_calculated_conflict_rates = combined_df_all.query("overlap_type in @legends")
        # df_calculated_conflict_rates.overlap_type.cat.categories = df_calculated_conflict_rates.overlap_type.cat.remove_unused_categories()
        df_calculated_conflict_rates.overlap_type.cat.remove_unused_categories(inplace=True)
        df_calculated_conflict_rates.boxplot(column='Simulated/analytical', by='overlap_type', ax=axs[1], rot=45)
        axs[1].set_ylim([0, 3])
        ...
        table_latex_code = combined_df_all.groupby(['overlap_type', 'has_conflicts'], sort=False).size().unstack().assign(
        ...
        ).loc[sorting[::-1]].to_latex(float_format="{:g}".format)

        figure_filename = '{}_20180101-20180102-20180104-20180105-splits_[0-1-2-3]-S_h-{}-S_v-{}-t_l-{:.4f}{}.{}'.format(filename_prefix, S_h_in_nm, S_v, t_l, timeshift_suffix, 'png')
        scenario_string = "Combined days, $S_h$={}, $S_v$={}, $t_l$={}{}".format(S_h_in_nm, S_v, t_l_in_minutes,
                                                                               title_suffix)
        print("\subsection{{{}}}".format(scenario_string))
        print(r"""
\begin{table}[H]
    \centering
    \caption{""" + scenario_string + r"""}
""" + table_latex_code + r"""
\end{table}
...
\begin{figure}[H]
    \centering
    \subfloat{
    \includegraphics[width=0.9\linewidth]{figures/replay/""" + figure_filename + r"""}}
    \\
    \subfloat{
    \includegraphics[width=0.9\linewidth]{figures/replay/trend-""" + figure_filename + r"""}}   
    \caption{Conflict counts and rate prediction, all types. """ + scenario_string + r"""}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics{figures/replay/trend-converging-""" + figure_filename + r"""}
    \caption{Conflict rate prediction for \texttt{converging\_av} type. """  + scenario_string + r"""}
\end{figure}
            """)
    warnings.warn("Couldn't open following files: {}".format("\n".join(not_found)))
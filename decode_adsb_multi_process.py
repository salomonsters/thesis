...
parser.add_argument('--nproc', help="number of proceses", default=multiprocessing.cpu_count())
...
fout = '%s/%d/%d_%02d/ADSB_DECODED_%d%02d%02d.csv' % (outdir, year, year, month, year, month, day)
...
def process_chunk(fraw):
    ...
    df_pos_raw = df_raw[(df_raw['tc'].between(9, 18))|(df_raw['tc'].between(5, 8))].copy()
    ...
    df_merged.drop_duplicates(['icao', 'lat', 'lon', 'gs', 'trk', 'roc'], keep='last', inplace=True)
